...
shinyServer(function(input,output,session) {
  ...
  observe({ #make sure that allele count is not greater than the total number of possible alleles
    if (input$init.alleles > 2*input$size) {
      updateSliderInput(session,"init.alleles",value=2*input$size)
    }
  })
    ...
    results[1,as.character(input$init.alleles)] <- 1
  ...
  output$resultsPlot <- renderPlot({
    print(persp(results(),theta=70,phi=10,xlab="generation",ylab="allele count",zlab="proportion",col="skyblue"))
  })
